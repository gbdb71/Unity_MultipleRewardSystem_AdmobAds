...
public class RewardBox : MonoBehaviour {
	...
	TMP_Text remainingAdsBadgeText;
	...
	void Awake ( ) {
		...
		remainingAdsBadgeText = remainingAdsBadge.transform.GetChild ( 0 ).GetComponent <TextMeshProUGUI> ( );

		// Get watched ads Text UI element and save the default text of the button
		watchAdButtonText = watchAdButton.transform.GetChild ( 0 ).GetComponent <TextMeshProUGUI> ( );
	...
	void DrawRewardsUI ( ) {
		for ( int i = currentRewardIndex; i < TOTAL_REWARDS; i++ ) {
			...
			//Reward Amount UI
			// rewardsParent.GetChild ( i ).GetChild ( 2 ).GetComponent<TextMeshProUGUI>().text = reward.Amount.ToString ( );
	...
	IEnumerator DOFillAmount(float waitTime, float progressValue)
	{

		for(;;)
		{
		if(progressBarFill.fillAmount<progressValue)
			{
					// Debug.Log("DoCheck");
					// progressBarFill.fillAmount += 0.1f;
					progressBarFill.fillAmount += progressValue / waitTime * Time.deltaTime;
					// Debug.Log( progressValue / waitTime * Time.deltaTime);
	...
	public void AdClose ( ) {
		...
		if ( isAdWatched ) {
			...
			// progressBarFill.DOFillAmount ( progressValue, 1.5f ).OnComplete ( RewardUser );
			 StartCoroutine (  DOFillAmount(1.5f,progressValue) );

			// progressBarFill.fillAmount = 1.0f;
			// RewardUser();
